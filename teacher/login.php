...
function generateSessionToken($dbh, $userid, $uid) {
  ...
  $insertTokenSQL = "INSERT INTO tbltoken (UserToken, UserID, role_id) VALUES (:token, :userid, 2)";
...
if(isset($_POST['login'])) 
  {
    ...
    $sql ="SELECT TeacherName uname, Email, TeaID, ID, Password, is2FA FROM tblteacher WHERE UserName=:username";
...
if (isset($_POST['confirm'])) {
  ...
  if ($otp == $_SESSION['otp']) {
    $_SESSION['otp'] = 0;
    generateSessionToken($dbh, $_SESSION['teaid'], $_SESSION['uid']);
    $_SESSION['teaid'] = 0;
    $_SESSION['uid'] = 0;
  } else {
